...
def update_pptx(excel_path, template_path, output_path, month):
    ...
    for filename in os.listdir(slides_dir):
        ...
        if 'Daily Active Users' in content:
            ...
            # Create previous month box (Left side, aligned perfectly under the prev month header)
            new_box = orig_box.replace('id="34"', 'id="1034"').replace('name="TextBox 33"', 'name="TextBox Prev"')
            # 5505638 perfectly aligns it under the February-2026 header at x=7076002 (matching the right-side layout)
            new_box = re.sub(r'<a:off x="([0-9]+)" y="([0-9]+)"', r'<a:off x="5505638" y="1840950"', new_box)
